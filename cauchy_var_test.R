...
robustness.cauchynoise.simulation<-function(n, m, q, x){
  k<-4*length(ssize)
  ...
  for(m_i in 1:m){
    ...
    error<-numeric(n)
    error[y1labs == 0]<-rnorm(n - odds)
    error[y1labs == 1]<-rcauchy(odds)
...
results.nonoise<-robustness.cauchynoise.simulation(n, 1000, 0, 0)

noise<-c(0.01, 0.05, 0.1, 0.2, 0.5)

for(a in noise){
  res<-robustness.cauchynoise.simulation(n, 1000, a, 0)
  print(c("Level: ", a))
  print(res$var_mean)
